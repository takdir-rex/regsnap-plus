...
/**
 * An inflight logger that periodically flushed available buffers according to a policy. Due to
 * limitations of the asynchronous file api, we must flush each epoch sequentially. Otherwise we may
 * accidentally free a buffer that has not been written yet. This is because each file is output to
 * by a different thread.
 *
 * <p>We start a different file for each epoch so that it may be deleted upon completion.
 */
public class SpillableSubpartitionInFlightLogger implements InFlightLog {

    private static final Logger LOG =
            LoggerFactory.getLogger(SpillableSubpartitionInFlightLogger.class);
    ...
    public SpillableSubpartitionInFlightLogger(
            IOManager ioManager, BufferPool prefetchBufferPool, boolean eagerlySpill) {
    ...
    public void log(Buffer buffer, long epochID, boolean isFinished) {
        ...
        LOG.debug(
                "Logged a new buffer for epoch {} with refcnt {} and size {}",
                epochID,
    ...
    public InFlightLogIterator<Buffer> getInFlightIterator(long epochID, int ignoreBuffers) {
        LOG.info(
                "Creating InFlightLog iterator for epochID {} and skipping {} buffers",
                epochID,
                ignoreBuffers);
        ...
        synchronized (flushLock) {
            ...
            if (logToReplay.size() == 0) return null;

            this.currentIterator =
                    new SpilledReplayIterator(
                            logToReplay,
                            prefetchBufferPool,
                            ioManager,
                            flushLock,
    ...
    private void notifyFlushCompleted(long epochID) {
        synchronized (flushLock) {
            ...
            if (epoch != null && !epoch.stable()) epoch.notifyFlushCompleted();
        }
    }
    ...
    private void notifyFlushFailed(long epochID) {
        synchronized (flushLock) {
            Epoch epoch = slicedLog.get(epochID);
            if (epoch != null && !epoch.stable()) epoch.notifyFlushFailed();
        }
    }
    ...
    private BufferFileWriter createNewWriter(long epochID) {
        ...
        try {
            writer =
                    ioManager.createBufferFileWriter(
                            ioManager.createChannel(), new FlushCompletedCallback(this, epochID));
        } catch (IOException e) {
            throw new RuntimeException(
                    "Failed to create BufferFileWriter. Reason: " + e.getMessage());
        }
        return writer;
    }
    ...
    static class Epoch {
        ...
        public void notifyFlushFailed() {
            ...
            LOG.debug(
                    "Flush failed for buffer {} of epoch {}, keeping in memory",
                    nextBufferToCompleteFlushing,
                    epochID);
            nextBufferToCompleteFlushing++;
            ...
            //			} catch (IOException e) {
            //				throw new RuntimeException("Writer could not write buffer. Cause:" +
            // e.getMessage());
            //			}
            //		}
            //	}
